...
function afc_post_pagination() {
	...
	if (is_single()) {
		...
		$prev_format = $smof_data['afc_prev_format'];
		
		$prev_link = $smof_data['afc_prev_link'];
		
		$next_format = $smof_data['afc_next_format'];
		
		$next_link = $smof_data['afc_next_link'];
		
		
		if ($smof_data['afc_links_cat'] == 1) {
