...
function afc_author_info() { 
	...
	if (is_single()) {
		...
		//show author avatar
		if($smof_data['afc_author_info']['avatar'] == true){
		...
		if($smof_data['afc_author_info']['title'] == true){
			$name =  sprintf($smof_data['afc_author_title'], get_the_author_meta( 'display_name' ));
			$output .= '<h2>' . $name . '</h2>';
		}
		
		//show author biography
		if($smof_data['afc_author_info']['bio'] == true){       
			$output .= '<div class="author-info-bio">' . get_the_author_meta( 'description' ) . '</div>';
		}
		 
		//show author social profiles links of current author
		if($smof_data['afc_author_info']['social'] == true){      
				 $output .= afc_profile_buttons();
		}
		
		//show a link to all posts of the current author
		if($smof_data['afc_author_info']['posts_link'] == true){     
