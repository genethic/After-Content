...
function afc_related_content() {
    global $post;
	global $smof_data;
	$num = $smof_data['afc_number_posts'];
	
	if($smof_data['afc_related_type'] == 'cat' && is_single()) {
		...
		if( $the_query->have_posts() ):
			$posts = '<section class="related-post-wrapper"><h3>' . $smof_data['afc_related_title'] .  '</h3>';
			
			while ($the_query->have_posts()): $the_query->the_post();
				$posts .= '<div class="related-post">';
				
				if ($smof_data['afc_display_img'] == 1) {
	...
	if($smof_data['afc_related_type'] == 'tag' && is_single()){
